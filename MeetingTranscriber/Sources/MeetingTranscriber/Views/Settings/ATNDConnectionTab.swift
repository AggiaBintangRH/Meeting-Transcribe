...
struct ATNDConnectionTab: View {
    ...
    /// Writes this Mac's address on the array's network straight into the
    /// Interface field, so the value is visible and pinned rather than implied.
    ///
    /// Fills in two cases only: the field is empty, or it holds an address that
    /// is no longer on any of this Mac's interfaces (stale after DHCP or a
    /// subnet move). A stale value is worse than a blank one — it joins a group
    /// on an interface that does not exist and receives nothing without error.
    /// A value that IS still local is left alone: that is a deliberate override.
    private func fillInterfaceFromRouting() {
        guard let detected = ATNDBeamService.localAddressRouting(toDeviceAt: deviceIP)
        else { return }
        if interfaceIP.isEmpty || !ATNDBeamService.isLocalAddress(interfaceIP) {
            interfaceIP = detected
        }
    }
    ...
    var body: some View {
        Group {
            ...
            SettingBlock(title: "2 · UDP multicast notifications") {
                ...
                Text("Interface is THIS MAC's address on the array's network, never the array's Device IP. It is filled in automatically from the routing table once the Device IP above is set, and refilled if it ever goes stale. Edit it only to override that.")
                    .font(.system(size: 11))
                    .foregroundColor(Theme.textFaint)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        .onAppear { fillInterfaceFromRouting() }
        // The lookup is derived from the device address, so redo it whenever
        // that is edited — a half-typed address simply resolves to nothing and
        // leaves the field untouched.
        .onChange(of: deviceIP) { fillInterfaceFromRouting() }
    }
