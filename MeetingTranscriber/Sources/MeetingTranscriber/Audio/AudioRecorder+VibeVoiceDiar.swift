...
// The VibeVoice diarization engine (`diarization.engine == "campplus"`) — session
// configuration, the two whole-file stop passes (office and remote), their
// watchdogs, and this domain's own log writer.
//
// WHAT IT IS
// ----------
// A pipeline built around a speaker EMBEDDING model, because that is all VibeVoice
// is: Silero VAD → VibeVoice vectors over 2 s sliding sub-windows → spectral
...
extension AudioRecorder {
    ...
    /// Read the engine choice for this session and wire the VibeVoice callbacks.
    ...
    func configureVibeVoiceDiar() {
        ...
        vibeVoiceDiarizationActive = diarOn && engine == ModelLoader.vibeVoiceEngineID
        guard vibeVoiceDiarizationActive, let service = modelLoader.vibeVoiceDiar else { return }
        ...
        vibeVoiceDiarLog(batchLiveDiarizationActive
        ...
        service.onFinalResult = { [weak self] audioPath, localTurns, stream in
            Task { @MainActor in
                ...
                self.vibeVoiceDiarLog("FINAL PASS done (\(stream == .office ? "office" : "remote")) "
        ...
        service.onError = { [weak self] message, stream in
            Task { @MainActor in
                ...
                if self?.handleBatchLiveFailure(message, stream: stream,
                    ...
                    == true { return }
                self?.vibeVoiceDiarLog("FAILED (\(stream == .office ? "office" : "remote")): \(message)")
    ...
    func startVibeVoiceDiarDiarization(_ recording: URL) {
        guard let service = modelLoader.vibeVoiceDiar else {
            // Unreachable through `stop()` (which tests the service first), kept
            // so the leg can never be left un-settleable by a future edit.
            diarizationError = "The VibeVoice diarization sidecar is not running"
            finalDiarDone = true
        ...
        vibeVoiceDiarLog("FINAL PASS start — \(fmt(recordingElapsed))s of office audio, "
        ...
        finalDiarWatchdog = Task { [weak self] in
            ...
            await MainActor.run {
                ...
                self.vibeVoiceDiarLog("FINAL PASS TIMEOUT after \(Int(limit))s")
                self.diarizing = false
                self.diarizationError = "VibeVoice diarization timed out"
                self.finalDiarDone = true
                self.setStopStep("diarize", .failed("VibeVoice diarization timed out"))
    ...
    @discardableResult
    func startRemoteVibeVoiceDiarDiarization() -> Bool {
        ...
        let mode = Self.remoteStopMode(
            finalPass: UserDefaults.standard.object(forKey: "diarization.finalPass")
                as? Bool ?? true,
                                       ...
                                       hasDiarizationService: modelLoader.vibeVoiceDiar != nil,
                                       hasRemoteRecording: remoteRecordingURL != nil,
                                       tailSamples: remoteDiarAudio.count,
                                       supportsTail: false)
        guard let service = modelLoader.vibeVoiceDiar, mode == .full,
        ...
        vibeVoiceDiarLog("REMOTE FINAL PASS start — whole Remote WAV, "
                   + "num_speakers=\(numSpeakers == 0 ? "auto" : String(numSpeakers))")
        ...
        startRemoteDiarWatchdog(
            ...
            message: "Remote VibeVoice diarization timed out")
        return true
    }

    // MARK: - Log

    /// Append a line to `logs/vibevoice-diarization.log`.
    ///
    ...
    /// mistake.
    func vibeVoiceDiarLog(_ message: String) {
        PythonRuntime.appendAppLog(name: "vibevoice-diarization", message: message)
    }
}
