...
final class NemoEngineTests: XCTestCase {
    ...
    /// live labels, so NeMo's remote pass is ALWAYS full — `supportsTail: false`,
    /// exactly as spectral passes it. Both directions: the same inputs with the
    /// default still give `.tail`, which is what proves the parameter is what
    /// changed the answer.
